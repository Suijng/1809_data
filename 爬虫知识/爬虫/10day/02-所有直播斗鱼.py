...
drive.get('https://www.douyu.com/directory/all')
...
while True:
    ...
    da = html.xpath('//div[@class="items items01 item-data clearfix"]//div[@class="mes"]')
    for i in da:
        title = i.xpath('.//div[@class="mes-tit"]/h3/text()')[0].replace(' ', '')
        num = i.xpath('.//span[@class="dy-num fr"]/text()')
        if len(num) >0:
            num = num[0]
