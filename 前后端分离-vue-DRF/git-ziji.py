...
ubuntu虚拟环境安装 pip install pyspider
		sudo apt-get install phantomjs
	报错以后:ConfigurationError: Could not run curl-config: [Errno 2] No such file or directory: 'curl-config'
			ERROR: Command errored out with exit status 1: python setup.py egg_info Check the logs for full command output.
		重现：
		pip install pycurl出现这个问题
		解决方案：
