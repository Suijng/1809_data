...
class Weibo(object):
    ...
    def create_excel(self):
        ...
        if not os.path.isfile(self.excel_filename):
            ...
            titles = ['名字', '内容', '时间', '来源', '收藏', '转发', '评论','点赞', '关注', '粉丝', '微博']
    ...
    def parse_data(self, text, currentUrl):
        ...
        pool = ProcessPoolExecutor(10) # 开启10个进程
        for item in items:
            self.row += 1
            item_dict = {}
            # 用户
            ...
            result = pool.submit(self.send_request,detail_url,allow_redirects=False,meta=item_dict)
            result.add_done_callback(self.parse_detail)
        pool.shutdown()

            # 开启10个进程
            # pool = ThreadPoolExecutor(10)
            # # 将任务提交到线程池中,让线程池中的线程执行任务
            # result = pool.submit(self.get_detail, detail_url)
            # # add_done_callback添加回调方法
            # result.add_done_callback(self.parse_detail)
            ...
            # <script>FM.view({"n":'jscbks})</script>
    ...
    def parse_detail(self,future):
        ...
        print(detail_url)
        # htmla = self.send_request(future)

        pattern = re.compile('<script>FM.view\(({.*?})\)</script>', re.S)

        result = re.findall(pattern, detail_url)
        if len(result) > 0:
            for item in result:
                item = item.replace('<script>FM.view(', '').replace(')</script>', '').replace('<!--欧盟隐私协议弹窗-->','').replace(';</script>','').replace('\n', '')
                # print(item)
                try:
                    ...
                    if 'tb_counter' in html:
                        ...
                        nums = profile_etree.xpath('//table[@class="tb_counter"]//td[@class="S_line1"]//strong[1]/text()')
                        print(nums, self.row)
                        # #关注量
                        meta['guanzhu_nums'] = int(nums[0])
                        self.sheet.cell(self.row, 9).value = meta['guanzhu_nums']
                        meta['fensi_nums'] = int(nums[1])
                        self.sheet.cell(self.row, 10).value = meta['fensi_nums']
                        meta['weibo_nums'] = int(nums[2])
                        self.sheet.cell(self.row, 11).value = meta['weibo_nums']
                    elif 'ul_detail' in html:
                        print('个人简介部分的html')
                        profile_etree = etree.HTML(html)
                        # 解析个人信息
                        content_ul = profile_etree.xpath('//ul[@class="ul_detail"]//li//span[2]//text()')
                            # .replace('\t','').replace('\n','').replace('\r','')
                        print(content_ul)
                except Exception as err:
                    print('json数据异常', err)
